...
function pagesTemplate(e) {
  ...
  form.innerHTML = `
      ...
      <div class="row g-3 align-items-center mb-2">
        <div class="col-4">
          ...
          <input type="text" class="form-control" id="title" placeholder="Page" />
        </div>
        <div class="col-6">
          <label class="form-label" for="body">Body</label>
          <input type="text" class="form-control" id="body" placeholder="Optional HTML" />
        </div>
  ...
  createBtn.addEventListener('click', async (ev) => {
    ...
    progressInfo.textContent = 'Creating pages...';
    ...
    try {
      ...
      for (let i = 1; i <= number; i++) {
        requests.push({ domain, token, course_id, title: `${title} ${i}`, body, published });
      }
      const res = await window.axios.createPages({ requests });
      responseDiv.textContent = `Created ${res.successful.length} page(s)`;
    } catch (err) {
