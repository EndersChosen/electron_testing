...
// Restore Deleted Conversations UI and logic
...
async function getDeletedConversations(e) {
    ...
    let bulkUserIds = [];
    let outputFolder = '';
    // Track latest results for single-user export to avoid rebinding listeners
    let lastResultsForCsv = [];
    let lastUserIdForCsv = '';
    ...
    if (form.dataset.bound !== 'true') {
        searchBtn.addEventListener('click', async (evt) => {
            ...
            try {
                ...
                if (count > 0) {
                    // Save latest results for single export and show button
                    lastResultsForCsv = results;
                    lastUserIdForCsv = user_id;
                    exportBtn.hidden = false;
                } else {
                    // Hide button and clear cache
        ...
        // Bind single export button once; uses latest search results
        exportBtn.addEventListener('click', (e2) => {
            e2.preventDefault();
            e2.stopPropagation();
            if (!lastResultsForCsv || lastResultsForCsv.length === 0) return;
            ...
            // Ensure headers include all keys and include deleted_at explicitly
